...
export async function getOrCreateCustomerLoyalty(
  ...
  const { data: newLoyalty, error } = await (supabase as any)
    .from('customer_loyalty')
    .insert({
      user_id: userId,
...
export async function awardPointsForPurchase(
  ...
  try {
    ...
    const { data: transactionId, error } = await (supabase as any).rpc('award_points', {
    ...
    const { data: loyalty } = await (supabase as any)
      .from('customer_loyalty')
...
export async function redeemReward(
  ...
  try {
    ...
    // Deduct points
    const { data: transactionId, error: pointsError } = await (supabase as any).rpc('award_points', {
      p_customer_id: customerId,
...
export async function processReferral(
  ...
  try {
    ...
    // Update referral stats - get current values first
    const { data: currentLoyalty } = await (supabase as any)
      .from('customer_loyalty')
      .select('referrals_count, referral_points_earned')
    ...
    if (currentLoyalty) {
      await (supabase as any)
        .from('customer_loyalty')
        .update({
          referrals_count: (currentLoyalty.referrals_count || 0) + 1,
          referral_points_earned: (currentLoyalty.referral_points_earned || 0) + referralPoints,
        })
        .eq('customer_id', referrer.customer_id);
    }
...
export async function getLoyaltyDashboard(userId: string): Promise<{
  ...
  const tierMap = new Map<string, string>(
    tiers?.map((t: any) => [t.id as string, t.name as string]) || []
  );
  ...
  for (const record of loyaltyRecords || []) {
    ...
    const tierName: string = record.tier_id ? (tierMap.get(record.tier_id) || 'Unknown') : 'None';
    tierDistribution[tierName] = (tierDistribution[tierName] || 0) + 1;
  }

  // Get recent redemptions count
  const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
  const { count: recentRedemptions } = await (supabase as any)
    .from('reward_redemptions')
  ...
  const topReferrers = (loyaltyRecords || [])
    .filter((r: any) => r.referrals_count > 0)
