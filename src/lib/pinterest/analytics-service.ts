...
async function updateCopyTemplatePerformance(userId: string) {
  ...
  for (const template of templates) {
    ...
    if (stats && stats.length > 0) {
      const totalImpressions = stats.reduce((sum: number, p: any) => sum + (p.impressions || 0), 0);
      const totalSaves = stats.reduce((sum: number, p: any) => sum + (p.saves || 0), 0);
      const totalClicks = stats.reduce((sum: number, p: any) => sum + (p.clicks || 0), 0);
