...
import { uploadFile, getFileBuffer, generateStorageKey } from '@/lib/storage/storage-utils';
...
export async function createProduct(
  ...
  try {
    ...
    const { data: pricingDefaults } = await (supabase as any)
      .from('product_pricing')
      .select('*')
      .or(`user_id.eq.${userId},is_system.eq.true`)
      .order('is_system', { ascending: true }); // User pricing takes precedence

    const pricingMap = new Map(pricingDefaults?.map((p: any) => [p.size, p]) || []);

    // Create variants
    const variantsToInsert = request.variants.map((v) => {
      const defaultPricing = pricingMap.get(v.size) as any;
    ...
    if (request.imageIds && request.imageIds.length > 0) {
      ...
      if (validImages.length > 0) {
        await (supabase as any).from('product_images').insert(validImages);
      }
    }
...
export async function publishProductToShopify(
  ...
  try {
    ...
    const shopifyProduct = await shopifyClient.createProduct({
      ...
      options: [
        { name: 'Size', values: [...new Set(product.variants.map((v: ProductVariant) => v.size))] as string[] },
        ...(product.variants.some((v: ProductVariant) => v.frame_style)
          ? [{ name: 'Frame', values: [...new Set(product.variants.filter((v: ProductVariant) => v.frame_style).map((v: ProductVariant) => v.frame_style))] as string[] }]
          : []
        ),
      ] as any,
    });
    ...
    const shopifyProductData = shopifyProduct.product as any;
    await (supabase as any)
      .from('products')
      .update({
        shopify_product_id: String(shopifyProductData.id),
        shopify_handle: shopifyProductData.handle,
        status: 'active',
