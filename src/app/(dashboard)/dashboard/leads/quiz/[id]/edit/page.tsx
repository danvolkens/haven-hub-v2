...
export default function EditQuizPage() {
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      try {
        ...
        setSlug(quiz.slug);
        setRequireEmail(quiz.require_email);
        setIsPublished(quiz.status === 'active');

        // Convert questions from DB format
        const formattedQuestions = (quiz.questions || []).map((q: any, qi: number) => ({
          id: q.id || String(qi + 1),
          text: q.text,
          type: q.type || 'single',
          answers: (q.answers || []).map((a: any, ai: number) => ({
            id: a.id || `${qi + 1}${String.fromCharCode(97 + ai)}`,
            text: a.text,
        ...
        // Convert results from DB format
        const formattedResults = (quiz.results || []).map((r: any) => ({
      ...
      const formattedQuestions: QuizQuestion[] = generatedQuestions.map(
        (q: { text: string; answers: { text: string; scores: { grounding: number; wholeness: number; growth: number } }[] }, idx: number) => ({
          ...
          answers: q.answers.map((a, aidx) => ({
            id: `${Date.now() + idx}${String.fromCharCode(97 + aidx)}`,
            text: a.text,
            scores: a.scores,
          })),
        })
  ...
  const togglePublish = async () => {
    try {
      ...
      const response = await fetch(`/api/quiz/by-id/${params.id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast(error instanceof Error ? error.message : 'Failed to update quiz', 'error');
    } finally {
  ...
  if (isLoading) {
    return (
      <PageContainer title="Edit Quiz" description="Loading...">
    ...
    >
      <div className="max-w-3xl space-y-6">
        ...
        <Card>
          <CardHeader
            title="Questions"
            description="Edit questions with weighted answers for each collection"
          />
